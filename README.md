...
**Phenylalanine** — Stage 2 (결합에너지) → Stage 3 (선택도)로 순위 교정

Stage 2 결합에너지만으로는 보론산이 과대평가되므로, **Stage 3 선택도**가 최종 순위이다.

*Stage 2: 결합에너지 순위 (ωB97XD)*

| 순위 | Monomer | bsse_dE (template) | 실험 IF | 비고 |
|------|---------|-------------------|--------|------|
| 1 | ACM | -31.888 | ~1.5 | |
| 2 | 4VB | -29.925 | ~2.4 | 공유결합 MIP |
| 3 | APB | -28.106 | ~2.0 | 공유결합 MIP |
| 4 | OPD | -25.591 | ~3.2 | |
| 5 | MAA | -24.088 | ~2.8 | |
| 6 | PYR | -21.848 | ~1.2 | |

Spearman ρ = -0.029 (IF와 상관 없음)

*Stage 3: 선택도 순위 (ωB97XD, 5종 interferent)*

| 순위 | Monomer | ΔΔE | 실험 IF | 논문 순위 | 비고 |
|------|---------|------|--------|----------|------|
| 1 | 4VB | -9.17 | ~2.4 | 3위 | 공유결합 MIP |
| 2 | ACM | -8.21 | ~1.5 | 5위 | |
| 3 | **OPD** | **-5.20** | **~3.2** | **1위** | **핵심 후보** |
| 4 | MAA | -4.48 | ~2.8 | 2위 | |
| 5 | PYR | -2.38 | ~1.2 | 7위 | **최하위권** |
| 6 | APB | -2.38 | ~2.0 | 4위 | 공유결합 → 선택도 낮음 |

Spearman ρ = **+0.257** (Stage 2 대비 +0.286 개선)

**Stage 3의 효과:**
- APB(보론산)가 3위→6위로 하락 — interferent에도 비선택적으로 강하게 결합 (자동 교정)
- OPD가 4위→3위로 상승 — template에만 선택적으로 결합
- Stage 2(ρ=-0.03) → Stage 3(ρ=+0.26): **상관관계 방향 전환**

### 14.6 종합 판정

| 검증 항목 | Stage | 결과 | 판정 |
|-----------|-------|------|------|
| Heptachlor IF 순위 (ωB97XD) | Stage 2 | Spearman ρ = **1.000** | **PASS** |
| DDT IF 순위 (ωB97M-V) | Stage 2 | Spearman ρ = **1.000** | **PASS** |
| DDT IF 1위 예측 (적응형) | Stage 2 | 4VP = 1위 | **PASS** |
| Phe OPD > PYR 방향성 | Stage 3 | OPD=3위, PYR=5위 | **PASS** |
| Phe PYR 최하위권 | Stage 3 | PYR = 5~6위/6개 | **PASS** |
| Phe 보론산 교정 (APB) | Stage 3 | Stage 2: 3위 → Stage 3: 6위 | **PASS** |
| Phe Stage 3 순위 개선 | Stage 2→3 | ρ: -0.029 → **+0.257** | **PASS** |
| 보론산(4VB, APB) 분류 | Stage 2 | 공유결합 MIP 자동 감지 | **PASS** |
...
### 14.9 Stage 3 선택도 검증 결과 (Phenylalanine)

Stage 2의 결합에너지 순위가 실험 IF와 맞지 않더라도, Stage 3에서 monomer-interferent 결합에너지를 추가 계산하면 순위가 교정되는지를 검증했다.

**계산 데이터**: 6 monomer × 5 interferent × 2 범함수 = 60개 DFT
- Monomers: OPD, MAA, 4VB, APB, ACM, PYR
- Interferents: Tyrosine, Valine, Leucine, Isoleucine, Dopamine (Mukasa 2023 Fig. 2g)

#### ωB97XD: monomer-interferent 결합에너지 (bsse_dE, kcal/mol)

| Monomer | E(m-Phe) | E(m-Tyr) | E(m-Val) | E(m-Leu) | E(m-Ile) | E(m-Dop) | avg(m-int) | ΔΔE |
|---------|---------|---------|---------|---------|---------|---------|-----------|------|
| OPD | -25.59 | -23.59 | -19.04 | -19.15 | -19.18 | -21.01 | -20.39 | **-5.20** |
| MAA | -24.09 | -21.92 | -15.71 | -20.19 | -19.79 | -20.42 | -19.61 | **-4.48** |
| 4VB | -29.93 | -25.25 | -18.09 | -18.56 | -21.39 | -20.47 | -20.75 | **-9.17** |
| APB | -28.11 | -26.66 | -25.27 | -25.18 | -26.93 | -24.61 | -25.73 | **-2.38** |
| ACM | -31.89 | -27.73 | -19.34 | -27.02 | -22.05 | -22.23 | -23.68 | **-8.21** |
| PYR | -21.85 | -23.55 | -18.93 | -19.16 | -17.90 | -17.80 | -19.47 | **-2.38** |

※ ΔΔE = E(monomer-template) - avg[E(monomer-interferent)]. 음수일수록 template에 선택적.

#### Stage 2 → Stage 3 순위 변화 (ωB97XD)

| 순위 | 실험 IF | Stage 2 (결합에너지) | Stage 3 (선택도) |
|------|---------|-------------------|-----------------|
| 1 | **OPD** (3.2) | ACM | **4VB** |
| 2 | **MAA** (2.8) | 4VB | **ACM** |
| 3 | 4VB (2.4) | APB | **OPD** |
...
| 지표 | Stage 2 | Stage 3 | 변화 |
|------|---------|---------|------|
| Spearman ρ vs IF | -0.029 | **+0.257** | **+0.286 개선** |
| OPD 위치 | 4위 | **3위** | 1단계 상승 |
| PYR 위치 | 6위 | 5위 | 유지 (하위권) |
| APB (보론산) | 3위 | **6위** | **3단계 하락 (교정됨)** |
| OPD > PYR | ✓ | ✓ | 유지 |

**핵심 발견**: Stage 3가 **보론산 문제를 자동으로 교정**한다. APB는 Stage 2에서 3위(강한 결합)였지만, Stage 3에서 6위(낮은 선택도)로 하락한다. 이는 APB의 B(OH)₂가 template(Phe)뿐 아니라 **interferent에도 비선택적으로 강하게 결합**하기 때문이다 (avg E(m-int) = -25.73, 다른 monomer보다 4~6 kcal/mol 더 강함).

#### ωB97M-V 결과 비교

| 지표 | Stage 2 | Stage 3 | 변화 |
|------|---------|---------|------|
| Spearman ρ vs IF | -0.029 | **+0.143** | +0.172 개선 |
| OPD 위치 | 4위 | **3위** | 1단계 상승 |
| APB (보론산) | 3위 | **6위** | 3단계 하락 (교정됨) |

두 범함수 모두에서 Stage 3가 순위를 개선하며, ωB97XD가 더 큰 개선을 보인다.

#### Stage 3의 한계

Spearman ρ = +0.257은 높지 않다. OPD가 실험 1위이지만 계산 3위인 이유:
- ACM(-8.21)과 4VB(-9.17)의 ΔΔE가 OPD(-5.20)보다 큰 것은, DFT 레벨에서 이 monomer들이 interferent와의 결합 대비 template 결합이 더 선택적으로 계산되기 때문
- 실험에서는 용매 효과, 중합 조건, cross-linker 영향 등 추가 요인이 IF에 기여하며, Stage 4(MD 시뮬레이션)에서 이를 보완
